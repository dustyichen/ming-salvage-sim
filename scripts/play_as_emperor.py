...
策略原则：
- 每月最多召见 3 位大臣，每位问 2-3 轮后立即让其拟旨退下。召够 3 人且有草案就必须退朝颁诏，不要拖延。
- 召见后先问情况、再问对策，最后让其拟旨。
- 拟旨前要确认大臣方案对得上你的目标。
- 颁诏前确认草案 ≥1 条。
- 钱粮有限，不要每旨都拨内库。
...
def run(
    ...
    # emperor agent 用 playtest 配置（args.base_url/model），子进程 main.py 让其按自己的
    # CLI_*（默认 deepseek）跑——故不覆盖 CLI_*。仅写 OPENAI_*/MING_SIM_DB 兜底兼容旧路径。
    env = os.environ.copy()
