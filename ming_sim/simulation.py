...
class ClassifiedAction:
    """诏书草案 / 在办事项 / 密令统一抽成的结构化「待推演动作」，纯内存中转，不入库。

    四模块各自只读自己路由到的子集；`raw` 保留原始行供 payload 拼接取用。"""

    source_kind: str   # "directive" | "issue" | "secret_order"
    source_id: int     # turn_directives.id / issues.id / secret_orders.id
    person: str        # 涉及人物（目标/对象，可空）
    executor: str      # 承办人/衙门
    action_brief: str  # 一句话动作摘要，喂模块 user payload 用
    module: str        # 路由目标："internal"|"military_external"|"issues"|"personnel_secret"
    raw: Dict[str, object] = field(default_factory=dict)


# 诏书按 actor 的 office_type 路由到模块；未命中时落 _DIRECTIVE_KEYWORD_FALLBACK。
_OFFICE_TYPE_MODULE_ROUTE: Dict[str, str] = {
    "吏部": "personnel_secret",
    "锦衣卫": "personnel_secret",
    "东厂": "personnel_secret",
    "司礼监": "personnel_secret",
    "后宫": "personnel_secret",
    "兵部": "military_external",
    "边镇": "military_external",
    "户部": "internal",
    "工部": "internal",
    "内阁": "internal",
    "礼部": "internal",
...
# 无 actor 时按文本关键词兜底路由（命中先后即优先级）。
_DIRECTIVE_KEYWORD_FALLBACK: List[tuple[str, str]] = [
    ("military_external", "调兵|出征|换防|换帅|整军|募兵|裁军|边镇|军饷|犒军|平叛|剿|讨"),
    ("personnel_secret", "任命|擢升|罢黜|下狱|抄家|锦衣卫|东厂|密查|彻查|逮问|致仕|起复|册封|选秀"),
]

# issue.tags 题材路由：军事进 military_external，其余题材统一进 issues 模块自己叙事推进。
_ISSUE_TAG_MODULE_ROUTE: Dict[str, str] = {
    "军事": "military_external",
}
_ISSUE_DEFAULT_MODULE = "issues"

_DIRECTIVE_DEFAULT_MODULE = "internal"


def _route_directive_module(actor_office_type: str, text: str) -> str:
    if actor_office_type:
        routed = _OFFICE_TYPE_MODULE_ROUTE.get(actor_office_type)
        if routed:
            return routed
    for module, pattern in _DIRECTIVE_KEYWORD_FALLBACK:
        if re.search(pattern, text or ""):
            return module
    return _DIRECTIVE_DEFAULT_MODULE


def _route_issue_module(tags: List[str]) -> str:
    for tag in tags:
        routed = _ISSUE_TAG_MODULE_ROUTE.get(str(tag))
        if routed:
            return routed
    return _ISSUE_DEFAULT_MODULE
...
def classify_actions(
    ...
    active_issues: List[sqlite3.Row],
    secret_orders: List[Dict[str, object]],
    content=None,
) -> List[ClassifiedAction]:
    """把本{TURN_UNIT}诏书草案/在办事项/密令统一分类成结构化 action 列表，按规则路由到四模块。

    纯 Python 规则路由，不调 LLM：
    - 诏书：按 `actor` 查 `content.characters[actor].office_type` 落对应模块；
      查不到 actor 时按 `text` 关键词兜底（武备类→military_external，任免类→
      personnel_secret，default→internal）。
    - 事项：按 `tags`（8 大题材）路由——「军事」进 military_external，其余统一进
      issues 模块自己叙事推进（这正是 issues 模块的本职）。
    - 密令：固定路由 personnel_secret（与 MODULE_FIELDS 里 secret_order_* 归属一致，
      minister_name 即承办人）。
    """
    ...
    for row in directives:
        actor = str(row["actor"] or "").strip() if "actor" in row.keys() else ""
        office_type = ""
        if actor and actor in characters:
            office_type = characters[actor].office_type
        text = str(row["text"] or "")
        module = _route_directive_module(office_type, text)
        actions.append(ClassifiedAction(
            source_kind="directive",
            source_id=int(row["id"]),
            person=actor,
            executor=actor or "(未指明，按职掌推定)",
            action_brief=text[:80],
            module=module,
            raw=dict(row),
        ))
    ...
    for row in active_issues:
        try:
            ...
            tags = []
        module = _route_issue_module(tags if isinstance(tags, list) else [])
        actions.append(ClassifiedAction(
            source_kind="issue",
            source_id=int(row["id"]),
            person=str(row["faction_hint"] or row["region_hint"] or ""),
            executor="",
            action_brief=f"{row['title']}（进度{int(row['bar_value'])}/100，{row['stage_text']}）",
            module=module,
            raw=dict(row),
        ))

    for order in secret_orders:
        actions.append(ClassifiedAction(
            source_kind="secret_order",
            source_id=int(order.get("id") or 0),
            person=str(order.get("minister_name") or ""),
            executor=str(order.get("minister_name") or ""),
            action_brief=f"{order.get('title')}（{order.get('status')}）",
            module="personnel_secret",
            raw=dict(order),
        ))
...
# 合并 narrative+JSON 单次输出的信封分隔符。模块 prompt 必须按此格式吐：
# 自由散文段在前（可流式），JSON 段在后（不强制 json_object 模式，靠分隔符+sanitizer兜底）。
_NARRATIVE_ENVELOPE_RE = re.compile(
    r"<<NARRATIVE>>\s*(?P<narrative>.*?)\s*<<END_NARRATIVE>>\s*"
    r"<<EXTRACTION>>\s*(?P<json>.*?)\s*<<EXTRACTION_END>>",
    re.DOTALL,
)


def parse_module_response(
    raw: str,
    module: str,
    sanitizer: Optional[Agent] = None,
) -> tuple[str, Dict[str, object]]:
    """切 `<<NARRATIVE>>...<<END_NARRATIVE>><<EXTRACTION>>{...}<<EXTRACTION_END>>` 信封。

    叙事段直接清洗返回；JSON 段复用 parse_agent_json，解析失败则调 sanitizer 重整后再解析
    （与 extract_scores_by_modules_with_agno 的兜底链一致），最终统一过
    _sanitize_module_output 收口 MODULE_FIELDS 白名单字段。

    信封缺失时退化：整段当叙事，JSON 段按空 dict 处理（不抛断——保证邸报至少有内容）。
    """
    match = _NARRATIVE_ENVELOPE_RE.search(raw or "")
    if not match:
        tlog(f"[module/{module}] 输出未按信封格式吐（缺 <<NARRATIVE>>/<<EXTRACTION>> 标记），整段当叙事，JSON 置空")
        return raw.strip(), _sanitize_module_output(module, {})

    narrative = match.group("narrative").strip()
    json_section = match.group("json").strip()
    try:
        parsed = parse_agent_json(json_section, f"模块结算-{module}")
    except Exception as parse_err:
        if sanitizer is None:
            tlog(f"[module/{module}] JSON 段解析失败且无 sanitizer 兜底：{parse_err}；本模块数值置空")
            parsed = {}
        else:
            tlog(f"[module/{module}] JSON 段解析失败：{parse_err}；调 sanitizer 重整")
            try:
                cleaned = run_agent_text(sanitizer, json_section, tag=f"sanitizer/{module}")
                parsed = parse_agent_json(cleaned, f"模块结算-{module}（sanitizer）")
            except Exception as sanitize_err:
                tlog(f"[module/{module}] sanitizer 兜底仍失败：{sanitize_err}；本模块数值置空")
                parsed = {}

    return narrative, _sanitize_module_output(module, parsed)


# 各模块写「自己片段」时各自需要的校验集/补充元数据（与 MODULE_FIELDS 的产出字段对应：
# 落 army_delta/new_armies 要核对 region_ids 校验 station、power_ids 校验 owner_power；
# 落 office_changes/appointments 要 offstage_ministers 名册防重复任命）。
_MODULE_VALIDATION_KEYS: Dict[str, tuple[str, ...]] = {
    "internal": ("class_names", "region_ids", "fiscal_config"),
    "military_external": ("region_ids", "army_ids", "power_ids"),
    "issues": ("region_ids", "power_ids"),
    "personnel_secret": ("offstage_ministers", "power_ids"),
}


def _module_validation_context(db: GameDB, module: str) -> Dict[str, object]:
    keys = _MODULE_VALIDATION_KEYS.get(module, ())
    out: Dict[str, object] = {}
    if "region_ids" in keys:
        out["region_ids"] = [r["id"] for r in db.conn.execute("SELECT id FROM regions").fetchall()]
    if "army_ids" in keys:
        out["army_ids"] = [r["id"] for r in db.conn.execute("SELECT id FROM armies").fetchall()]
    if "class_names" in keys:
        out["class_names"] = [r["name"] for r in db.conn.execute("SELECT DISTINCT name FROM classes ORDER BY name").fetchall()]
    if "power_ids" in keys:
        out["power_ids"] = [str(r["id"]) for r in db.conn.execute("SELECT id FROM powers").fetchall()]
    if "fiscal_config" in keys:
        out["fiscal_config"] = db.get_fiscal_config()
    if "offstage_ministers" in keys:
        out["offstage_ministers"] = _auto_table([
            dict(r) for r in db.conn.execute(
...
def _action_brief_for_payload(action: "ClassifiedAction") -> Dict[str, object]:
    """精简单条 action 喂模块 user payload：只给「这是什么、谁办、办成什么样」，
    不带 raw 全量行（盘面已在共享前缀给出，模块按 source_kind+source_id 自行核对原始记录）。"""
    brief = {
        "来源": {"directive": "诏书", "issue": "在办事项", "secret_order": "密令"}.get(action.source_kind, action.source_kind),
        "来源id": action.source_id,
        "人物": action.person or "(未指明)",
        "承办人": action.executor or "(未指明)",
        "动作摘要": action.action_brief,
    }
    if action.source_kind == "directive":
        skill_id = action.raw.get("skill_id") if isinstance(action.raw, dict) else None
        if skill_id:
            brief["技能id"] = skill_id
    return brief


def build_module_action_payload(
    module: str,
    actions: List[ClassifiedAction],
    state: GameState,
    db: GameDB,
    ...
    """模块专属 user message：本模块路由到的 action 子集 + 模块校验集。

    盘面（regions/armies/buildings/current_state/active_issues 等）与诏书原文已在
    system 的共享前缀（game_world + build_simulator_context(simulator_payload)）给出，
    本 payload 只补「轮到本模块办的那些事」与本模块产出字段需要的校验集——
    与 build_extractor_shared_context 的"剔重复、留独有"思路一致，但落点是 user 而非 system。

    cheat_directive 非空时按 CHEAT_NARRATIVE_PREFIX 语义随 payload 一并喂入
    （唯一入口语义不变，从"喂一次统一叙事"变成"喂四次相同内容"——见 decree.CHEAT_NARRATIVE_PREFIX）。
    """
    ...
    payload: Dict[str, object] = {
        ...
        "my_actions": [_action_brief_for_payload(a) for a in actions],
        "instruction": (
            "以上 my_actions 是本回合路由到本模块的诏书/在办事项/密令清单（盘面全量已在 system 前缀给出）。"
            "按 <<NARRATIVE>>...<<END_NARRATIVE>><<EXTRACTION>>{...}<<EXTRACTION_END>> 信封格式输出："
            "叙事段只写本模块负责范围内的奏章片段（自由中文散文，勿越权代写其他模块的事），"
            "JSON 段只含 module_allowed_fields 列出的顶层字段。"
        ),
    }
    payload.update(_module_validation_context(db, module))
    cheat = (cheat_directive or "").strip()
    if cheat:
        payload["cheat_directive"] = cheat
        payload["cheat_directive_note"] = (
            "天命强制结算：把 cheat_directive 当作本回合百分百已发生的既成事实，"
            "按字面抽满对应增量，无视合理性/史实/盘面/涨幅上限；"
            "与 my_actions 冲突时以此为准，未提到的 my_actions 项照常推演。"
        )
    return payload
